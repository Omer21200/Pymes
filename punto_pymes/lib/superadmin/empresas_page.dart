...
class _EmpresasPageState extends State<EmpresasPage> {
  ...
  final TextEditingController _horaEntradaAlmuerzoCtrl =
      TextEditingController();
  final TextEditingController _toleranciaCtrl = TextEditingController(
    text: '15',
  );
        ...
        _companies = resList
            .map<Map<String, dynamic>>(
              (e) => {
                ...
                'hora_entrada_almuerzo': e['hora_entrada_almuerzo'] ?? '',
  ...
  Future<void> _createEmpresa() async {
    ...
    if (nombre.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Nombre es obligatorio'),
    ...
    try {
      ...
      if (_logoFile != null) {
        try {
          ...
          final fileName =
              'logos/${DateTime.now().millisecondsSinceEpoch}_${_logoFile!.name}';
          ...
          if (kIsWeb) {
            ...
            debugPrint(
              'Subida de logo no soportada desde web en esta versión del cliente.',
            );
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Subida de logo no soportada desde web'),
              ),
            );
          } else {
            ...
            try {
              await supabase.storage
                  .from('logos')
                  .upload(fileName, File(_logoFile!.path));
            } catch (e) {
          ...
          final dynamic pub = supabase.storage
              .from('logos')
          ...
          if (pub == null) {
            ...
            url = (pub['publicUrl'] ?? pub['publicURL'] ?? pub['url'] ?? '')
                .toString();
          } else {
            url = pub.toString();
          }
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('No se pudo subir el logo: ${e.toString()}'),
      ...
      final payload = {
        ...
        'email': _emailCtrl.text.trim().isNotEmpty
            ...
            : null,
        'hora_entrada': _horaEntradaCtrl.text.trim().isNotEmpty
            ? _horaEntradaCtrl.text.trim()
            : null,
        'hora_salida': _horaSalidaCtrl.text.trim().isNotEmpty
            ? _horaSalidaCtrl.text.trim()
            : null,
        'hora_almuerzo': _horaAlmuerzoCtrl.text.trim().isNotEmpty
            ? _horaAlmuerzoCtrl.text.trim()
            : null,
        'hora_entrada_almuerzo': _horaEntradaAlmuerzoCtrl.text.trim().isNotEmpty
            ? _horaEntradaAlmuerzoCtrl.text.trim()
            : null,
        'logo_url': _logoUrl,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error creando empresa: $e'),
          backgroundColor: Colors.red,
        ),
      );
    } finally {
  ...
  Future<void> _deleteCompany(String id) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text('¿Eliminar esta empresa?'),
        actions: [
          TextButton(
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error eliminando empresa: $e'),
          backgroundColor: Colors.red,
        ),
      );
    }
  ...
  Future<void> _editCompany(Map<String, dynamic> company) async {
    ...
    final TextEditingController rucCtrl = TextEditingController(
      text: company['ruc']?.toString() ?? '',
    );
    final TextEditingController direccionCtrl = TextEditingController(
      text: company['direccion']?.toString() ?? '',
    );
    final TextEditingController telefonoCtrl = TextEditingController(
      text: company['telefono']?.toString() ?? '',
    );
    final TextEditingController emailCtrl = TextEditingController(
      text: company['email']?.toString() ?? '',
    );
    final TextEditingController horaEntradaCtrl = TextEditingController(
      text: company['hora_entrada']?.toString() ?? '',
    );
    final TextEditingController horaSalidaCtrl = TextEditingController(
      text: company['hora_salida']?.toString() ?? '',
    );
    final TextEditingController horaAlmuerzoCtrl = TextEditingController(
      text: company['hora_almuerzo']?.toString() ?? '',
    );
    final TextEditingController horaEntradaAlmuerzoCtrl = TextEditingController(
      text: company['hora_entrada_almuerzo']?.toString() ?? '',
    );
    final TextEditingController toleranciaCtrl = TextEditingController(
      text: (company['tolerancia_minutos'] ?? 0).toString(),
    );
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(
                      child: TextField(
                        controller: horaAlmuerzoCtrl,
                        readOnly: true,
                        decoration: InputDecoration(
                          hintText: 'Hora Almuerzo',
                          border: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(8),
                          ),
                        ),
                    ...
                    Expanded(
                      child: TextField(
                        controller: horaEntradaAlmuerzoCtrl,
                        readOnly: true,
                        decoration: InputDecoration(
                          hintText: 'Entrada Almuerzo',
                          border: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(8),
                          ),
                        ),
                    ...
                    onPressed: () async {
                      final payload = {
                        ...
                        'email': emailCtrl.text.trim().isNotEmpty
                            ? emailCtrl.text.trim()
                            : null,
                        'hora_entrada': horaEntradaCtrl.text.trim().isNotEmpty
                            ? horaEntradaCtrl.text.trim()
                            : null,
                        'hora_salida': horaSalidaCtrl.text.trim().isNotEmpty
                            ? horaSalidaCtrl.text.trim()
                            : null,
                        'hora_almuerzo': horaAlmuerzoCtrl.text.trim().isNotEmpty
                            ? horaAlmuerzoCtrl.text.trim()
                            : null,
                        'hora_entrada_almuerzo':
                            horaEntradaAlmuerzoCtrl.text.trim().isNotEmpty
                            ? horaEntradaAlmuerzoCtrl.text.trim()
                            : null,
                        'tolerancia_minutos':
                            int.tryParse(toleranciaCtrl.text) ?? 0,
                      };
                      try {
                        await supabase
                            .from('empresas')
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text('Error actualizando empresa: $e'),
                            backgroundColor: Colors.red,
                          ),
                        );
                      }
              ...
              child: Column(
                ...
                children: [
                    ...
                    children: [
                        ...
                        child: TextField(
                          ...
                          onTap: () async {
                            ...
                            if (_horaEntradaCtrl.text.isNotEmpty) {
                              ...
                              if (parts.length == 2) {
                                final h =
                                    int.tryParse(parts[0]) ?? initial.hour;
                                final m =
                          ...
                          decoration: InputDecoration(
                            hintText: 'Hora de Entrada (HH:MM)',
                            border: OutlineInputBorder(
                              borderRadius: BorderRadius.circular(8),
                            ),
                          ),
                      ...
                      SizedBox(
                        ...
                        child: DropdownButtonFormField<String>(
                          ...
                          onChanged: (v) {
                            if (v != null)
                              setState(() => _toleranciaCtrl.text = v);
                          },
                          decoration: InputDecoration(
                            border: OutlineInputBorder(
                              borderRadius: BorderRadius.circular(8),
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: 'Entrada Almuerzo (HH:MM)',
                      border: OutlineInputBorder(
                        borderRadius: BorderRadius.circular(8),
                      ),
                    ),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          Text(
                            'Entrada: ${c['hora_entrada'] ?? '-'} (±${c['tolerancia_minutos'] ?? 0} min)',
                            style: const TextStyle(color: Colors.grey),
                          ),
                          Text(
                            'Registrada: --',
                      ...
                      Align(
                        ...
                        child: TextButton(
                          onPressed: () => Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (_) => EmpresaDetailsPage(company: c),
                            ),
                          ),
